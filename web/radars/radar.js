(() => {
  ...
  function buildStandaloneReport(data, generatedAt = new Date()) {
    ...
    const matrixPoints = rankedScenarios.map((scenario) => `<button type="button" data-export-target="${escapeHtml(scenario.id)}" aria-pressed="${scenario === rankedScenarios[0] ? 'true' : 'false'}" style="left:${scenario.matrix.x}%;top:${scenario.matrix.y}%"><b>${escapeHtml(scenario.number)}</b><span>${escapeHtml(scenario.shortTitle)}</span></button>`).join('');
    ...
    const scenarios = data.scenarios.map((scenario) => `<article class="export-scenario" id="export-${escapeHtml(scenario.id)}" data-priority="${escapeHtml(scenario.priority)}" data-category="${escapeHtml(scenario.category)}"><details open><summary tabindex="-1"><span>${escapeHtml(scenario.number)}</span><b>${escapeHtml(scenario.priority)}</b><h3>${escapeHtml(scenario.title)}</h3><strong>${scenario.scorecard.total} 分</strong></summary>${exportScorecard(scenario)}${exportScenarioDetails(scenario, data)}</details></article>`).join('');
